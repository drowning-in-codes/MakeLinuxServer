...
int main() {
  ...
  client_addr.sin_port = htons(2025);
  int retcode = inet_pton(AF_INET, "127.0.0.1", &client_addr.sin_addr);
  if (retcode != 1) {
    perror("inet_pton");
    close(client_fd);
    return -1;
  ...
  //   绑定套接字
  errif(bind(client_fd, (struct sockaddr *)&client_addr, sizeof(client_addr)) <
            0,
        "bind");
  // 连接服务器
