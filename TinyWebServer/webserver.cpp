...
void WebServer::log_write() {
  if (0 == m_close_log) {
    ...
    if (1 == m_log_write) {
      ...
      Log::get_instance()->init("./ServerLog", m_close_log, 200, 800000, 0);
    }
  }
}
...
void WebServer::eventListen() {
  //网络编程基础步骤
  m_listenfd = socket(PF_INET, SOCK_STREAM, 0); // 服务端监听套接字
  assert(m_listenfd >= 0);
  ...
  // 设置信号处理,当
  // 忽略pipe信号 尝试写入一个已关闭读端的管道或套接字时产生
  utils.addsig(SIGPIPE, SIG_IGN);
  // alarm信号 发生时 向pipe[1]发送数据,pipe[0]接收数据
  utils.addsig(SIGALRM, utils.sig_handler, false);
  //   SIGTERM 是一种标准的终止信号，用来请求一个进程正常退出。
  utils.addsig(SIGTERM, utils.sig_handler, false);
  // 设置 TIMESLOT秒后发送 SIGALRM 信号
  alarm(TIMESLOT);
...
void WebServer::dealwithwrite(int sockfd) {
  ...
  if (1 == m_actormodel) {
    ...
    while (true) {
      ...
      if (1 == users[sockfd].improv) {
        if (1 == users[sockfd].timer_flag) {
          // 写处理失败,将fd从epoll移除,close(fd)并移除timer
          deal_timer(timer, sockfd);
          users[sockfd].timer_flag = 0;
        }
        users[sockfd].improv = 0;
